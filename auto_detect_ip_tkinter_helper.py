...
def copy_ip():
    ...
    root.clipboard_append(ip_var.get())
...
def run_frida():
    ...
    # --- DYNAMIC REPLACEMENT ---
    # We replace the placeholder in the JS with the actual IP from the UI
    final_js = script_var.replace("1.1.1.1", current_ip)
    
    script_file = "frida_script_temp.js"
    with open(script_file, "w") as f:
        f.write(final_js)
    
    cmd = ["frida", "-U", "-l", script_file, "-f", pkg]
    try:
        subprocess.Popen(cmd)
        print(f"[*] Injecting into {pkg} with Burp IP: {current_ip}")
    except Exception as e:
        print(f"Error: {e}")
# --- UI Setup ---
root = tk.Tk()
root.title("Frida Control Center")
root.geometry("500x500")
root.configure(bg="#f5f5f7")

# Variables
ip_var = tk.StringVar(value="0.0.0.0")
pkg_var = tk.StringVar(value="com.example")

# --- THE JAVASCRIPT SCRIPT (Your hardcoded content) ---
script_var = """
/* Global variables */
var appId = null;
var appId_iOS = null;
...
style.configure("Header.TLabel", background="#ffffff", font=("Helvetica", 12, "bold"))
style.configure("Action.TButton", font=("Helvetica", 10, "bold"), padding=10)

main_container = tk.Frame(root, bg="#f5f5f7", padx=20, pady=20)
main_container.pack(expand=True, fill="both")

# Network Card
net_card = tk.Frame(main_container, bg="#ffffff", highlightthickness=1, highlightbackground="#dddddd", padx=20, pady=20)
net_card.pack(fill="x", pady=(0, 20))

ttk.Label(net_card, text="HOST NETWORK INTERFACE", style="Header.TLabel").pack(anchor="w")
tk.Label(net_card, textvariable=ip_var, font=("Courier", 20, "bold"), fg="#007AFF", bg="#ffffff").pack(pady=15)

btn_row = tk.Frame(net_card, bg="#ffffff")
btn_row.pack(fill="x")
ttk.Button(btn_row, text="Detect IP", command=detect_ip).pack(side="left", expand=True, fill="x", padx=5)
ttk.Button(btn_row, text="Copy IP", command=copy_ip).pack(side="left", expand=True, fill="x", padx=5)

# Target Card
cfg_card = tk.Frame(main_container, bg="#ffffff", highlightthickness=1, highlightbackground="#dddddd", padx=20, pady=20)
cfg_card.pack(fill="x", pady=(0, 20))

ttk.Label(cfg_card, text="TARGET PACKAGE", style="Header.TLabel").pack(anchor="w")
ttk.Entry(cfg_card, textvariable=pkg_var).pack(fill="x", pady=10)

# Action Button
ttk.Button(main_container, text="🚀 START FRIDA SESSION", style="Action.TButton", command=run_frida).pack(fill="x")
